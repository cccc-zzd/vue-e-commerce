<template>
	<div class="trade-container">
		...
		<div class="content">
			...
			<div class="detail">
				...
				<ul class="list clearFix" v-for="(order,index) in orderInfo.detailArrayList" :key="order.skuId">
					<li>
						<img :src="order.imgUrl" alt="" style="width:50px;height:50px">
					</li>
					<li>
						<p>
							{{ order.skuName }}</p>
		...
		<div class="money clearFix">
			<ul>
				<li>
					<b><i><!--{{ orderInfo.totalNum }}--> {{ totalcount }}</i>件商品，总商品金额</b>
		...
		computed: {
			...
			totalcount() {
				...
				if (this.orderInfo.detailArrayList) {
					this.orderInfo.detailArrayList.forEach(item => count += item.skuNum);
					return count
				} else {
					return count
				}
